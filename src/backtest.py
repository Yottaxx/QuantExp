    ...
    def next(self):
        ...
        for code in target_codes:
            ...
            if self.getposition(data).size == 0:
                ...
                if size > vol * 100 * self.p.min_volume_percent: size = int(
                    vol * 100 * self.p.min_volume_percent / 100) * 100
                if size >= 100: self.buy(data=data, size=size); self.hold_time[code] = 0; buy_cnt += 1
...
class WalkForwardBacktester:
    ...
    def generate_signal_matrix(self):
        ...
        results, batch_inputs, batch_meta = [], [], []
        print("正在批量推理历史数据...")
        ...
        if batch_inputs: self._flush_batch(model, batch_inputs, batch_meta, results)
    ...
    def _flush_batch(self, model, inputs, meta, res):
        ...
        if s.ndim == 0: s = [s]
        for i, score in enumerate(s): res.append((meta[i][0], meta[i][1], float(score)))
    ...
    def run(self, top_k=5):
        ...
        cerebro.addstrategy(ModelDrivenStrategy, signals=signals, top_k=top_k, hold_days=Config.PRED_LEN)
    ...
    def _generate_report(self, strat, cerebro):
        ...
        print(
            f"\n📊 [Walk-Forward 真实回测报告]\n区间: {self.start_date} ~ {self.end_date}\n资金: {self.initial_cash:,.0f} -> {final_val:,.2f}\n收益: {ret:.2%}\n夏普: {sharpe:.2f}\n回撤: {max_dd:.2%}")
